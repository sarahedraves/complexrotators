    ...
    def get_aperture_lc(self, data_type="raw", weighting=None, split=False, verbose=True):
        ...
        for r in rows:
            for c in cols:
                if weighting == "median":
                    weight = medvals[r][c]
                elif weighting == None: #we're here
                    weight = 1.0
                if split:
                    aperture_lc += weight*self.models[r][c].split_values_dict[data_type]
                else: #and here with data_type 'cpm_subtracted_flux'
                    print(self.models[r][c])
                    print(self.models[r][c].values_dict)
                    print(self.models[r][c].values_dict[data_type])
